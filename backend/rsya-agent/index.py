    ...
    try:
        ...
        if function_calls:
            ...
            if errors:
                ...
                # Для всех остальных функций (включая Google Sheets) - отдаём результаты обратно в Gemini
...
→ Получаешь ВСЕ строки и заголовки
→ АНАЛИЗИРУЕШЬ: какие колонки есть, где даты, где данные по Директу/рекламе

ШАГ 2: Понимаешь структуру
→ Смотришь на заголовки и первые строки
→ Определяешь какая колонка = даты (может называться "Дата", "Date", "Число" и т.д.)
→ Определяешь какая колонка = данные Директа (может быть "Директ", "Яндекс", "Yandex Direct", "Расход" и т.д.)
→ Находишь пустые ячейки в колонке с данными Директа

ШАГ 3: Извлекаешь даты для заполнения
→ Берёшь даты из строк где данные Директа пустые
→ Преобразуешь в формат YYYY-MM-DD (например: "01.12.2024" → "2024-12-01")

ШАГ 4: Получаешь статистику
→ Вызываешь get_direct_stats_by_dates(dates=[...]) для пропущенных дат
→ Получаешь: расход, клики, показы, конверсии

ШАГ 5: Заполняешь таблицу
→ Определяешь букву колонки (A=0, B=1, C=2 и т.д.)
...
⚠️ КРИТИЧЕСКИ ВАЖНО:
- ТЫ САМ анализируешь структуру таблицы! НЕ ищи только по словам "Дата" и "Директ"
- Смотри на СОДЕРЖИМОЕ первых строк чтобы понять что где
- Колонка с датами может называться как угодно, но содержать даты (01.12.2024, 2024-12-01 и т.д.)
- Колонка с Директом может называться по-разному, но быть пустой или с числами
- Всегда объясняй пользователю ЧТО ты понял из таблицы
...
def read_google_sheet(user_id: str, project_id: str, args: Dict) -> Dict:
    ...
    try:
        ...
        # Парсим структуру - ОТДАЁМ ВСЁ GEMINI, пусть он сам разбирается
        headers = values[0]
        rows = values[1:]
        
        print(f'📊 Headers from sheet: {headers}')
        
        # Собираем ВСЕ данные из таблицы (первые 50 строк для анализа)
        all_data = []
        for row_idx, row in enumerate(rows[:50]):
            row_dict = {}
            for col_idx, header in enumerate(headers):
                if col_idx < len(row):
                    row_dict[str(header)] = row[col_idx]
                else:
                    row_dict[str(header)] = ''
            row_dict['_row_number'] = row_idx + 2
        ...
        return {
            ...
            'message': f'Прочитано {len(all_data)} строк из таблицы. Заголовки: {", ".join(str(h) for h in headers)}'
        }
