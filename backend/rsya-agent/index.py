...
- Когда пользователь просит "покажи площадки", "выведи площадки", "список площадок", "почисти РСЯ", "проанализируй площадки" → НЕМЕДЛЕННО вызови analyze_rsya_platforms()
- Когда пользователь просит "покажи кампании", "список кампаний" → НЕМЕДЛЕННО вызови get_campaigns()
- НЕ СПРАШИВАЙ подтверждение, НЕ ОБЪЯСНЯЙ что собираешься сделать — СРАЗУ вызывай функцию!

ВАЖНО: После вызова analyze_rsya_platforms():
