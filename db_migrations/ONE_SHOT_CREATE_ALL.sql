-- Один скрипт: схема + все таблицы + индексы для PostgreSQL
-- Схема: t_p97630513_yandex_cleaning_serv (как в коде)
-- Запуск: psql "$DATABASE_URL" -f ONE_SHOT_CREATE_ALL.sql
...
CREATE TABLE IF NOT EXISTS t_p97630513_yandex_cleaning_serv.rsya_tasks (
    ...
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    combine_operator VARCHAR(20) DEFAULT 'AND'
);
...
-- block_queue (код использует колонку domain)
...
-- rsya_campaign_batches (структура под код: campaign_ids, total_batches)
...
-- rsya_campaign_locks (код: campaign_id, locked_by, expires_at)
...
-- telega_crm_projects
CREATE TABLE IF NOT EXISTS t_p97630513_yandex_cleaning_serv.telega_crm_projects (
    id SERIAL PRIMARY KEY,
    user_id INTEGER NOT NULL,
    name TEXT NOT NULL,
...
-- telega_crm_leads
CREATE TABLE IF NOT EXISTS t_p97630513_yandex_cleaning_serv.telega_crm_leads (
    id SERIAL PRIMARY KEY,
    project_id INTEGER NOT NULL,
    phone TEXT,
    name TEXT,
...
CREATE INDEX IF NOT EXISTS idx_block_queue_project_status ON t_p97630513_yandex_cleaning_serv.block_queue(project_id, status);
...
CREATE INDEX IF NOT EXISTS idx_rsya_pending_reports_status ON t_p97630513_yandex_cleaning_serv.rsya_pending_reports(status);
CREATE INDEX IF NOT EXISTS idx_clustering_projects_user_id ON t_p97630513_yandex_cleaning_serv.clustering_projects(user_id);
CREATE INDEX IF NOT EXISTS idx_subscriptions_user_id ON t_p97630513_yandex_cleaning_serv.subscriptions(user_id);
CREATE INDEX IF NOT EXISTS idx_partners_user_id ON t_p97630513_yandex_cleaning_serv.partners(user_id);
CREATE INDEX IF NOT EXISTS idx_telega_projects_user_id ON t_p97630513_yandex_cleaning_serv.telega_crm_projects(user_id);
CREATE INDEX IF NOT EXISTS idx_telega_leads_project_id ON t_p97630513_yandex_cleaning_serv.telega_crm_leads(project_id);
