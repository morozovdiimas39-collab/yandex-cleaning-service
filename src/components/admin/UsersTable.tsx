...
interface Props {
  users: User[];
  onUpdateUser: (userId: string, planType: string, days: number, manualProjectSlots?: number) => void;
...
export default function UsersTable({ users, onUpdateUser, onDeleteUser, onImpersonateUser }: Props) {
  const [editingUser, setEditingUser] = useState<string | null>(null);
  const [editData, setEditData] = useState({ planType: 'trial', days: 30, manualProjectSlots: 0 });

  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[920px] border-collapse">
        <thead><tr className="border-b border-slate-200 bg-slate-50/80">
          {['Пользователь', 'Тариф', 'Проекты', 'Доступ', 'Истекает', 'Регистрация', ''].map((label) => <th key={label} className="px-5 py-3 text-left text-xs font-medium uppercase text-slate-500 last:text-right">{label}</th>)}
        </tr></thead>
        <tbody className="divide-y divide-slate-100">
          {users.map((user) => {
            ...
            return <tr key={user.userId} className="transition-colors hover:bg-slate-50/70">
              ...
              <td className="px-5 py-4">{isEditing ? <div className="min-w-[170px]"><Input type="number" min="0" aria-label="Ручные проекты" value={editData.manualProjectSlots} onChange={(e) => setEditData({ ...editData, manualProjectSlots: Math.max(0, Number(e.target.value) || 0) })} className="h-9 w-28" /><div className="mt-1 text-xs text-slate-500">Доп. вручную</div></div> : <div className="text-sm text-slate-700"><div className="font-semibold text-slate-950">{user.projectLimit ?? '—'} всего</div><div className="mt-1 text-xs text-slate-500">оплачено: {user.paidProjectSlots ?? 0}, вручную: {user.manualProjectSlots ?? 0}</div></div>}</td>
              ...
              <td className="px-5 py-4 text-right">{isEditing ? <div className="flex items-center justify-end gap-2"><Input type="number" min="1" aria-label="Количество дней" value={editData.days} onChange={(e) => setEditData({ ...editData, days: Number(e.target.value) || 1 })} className="h-9 w-20" /><Button size="sm" onClick={() => { onUpdateUser(user.userId, editData.planType, editData.days, editData.manualProjectSlots); setEditingUser(null); }}><Icon name="Check" size={16} className="mr-1" />Сохранить</Button><Button size="icon" variant="outline" aria-label="Отменить" onClick={() => setEditingUser(null)}><Icon name="X" size={16} /></Button></div> : <div className="flex items-center justify-end gap-2"><Button size="sm" className="bg-emerald-600 text-white hover:bg-emerald-700" onClick={() => onImpersonateUser(user)}><Icon name="LogIn" size={15} className="mr-1.5" />Войти</Button><Button size="sm" variant="outline" onClick={() => { setEditingUser(user.userId); setEditData({ planType: user.planType === 'monthly' ? 'monthly' : 'trial', days: 30, manualProjectSlots: user.manualProjectSlots ?? 0 }); }}><Icon name="Pencil" size={15} className="mr-1.5" />Изменить</Button><Button size="icon" variant="ghost" className="text-slate-400 hover:bg-rose-50 hover:text-rose-600" aria-label="Удалить пользователя" onClick={() => onDeleteUser(user.userId)}><Icon name="Trash2" size={16} /></Button></div>}</td>
            </tr>;
          })}
        </tbody>
      </table>
